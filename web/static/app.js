...
async function loadAnnouncements() {
  if (!state.guildId) return;
  const announcements = await api(`/api/announcements?guild_id=${state.guildId}`);
  const list = $("announcement-list");
  list.innerHTML = "";
  announcements.forEach((a) => {
    const li = document.createElement("li");
    const span = document.createElement("span");
    span.textContent = `${String(a.hour).padStart(2, "0")}:00 - ${a.url}`;
    const btn = document.createElement("button");
    btn.textContent = "✕";
    btn.onclick = async () => {
      await api(`/api/announcements/${a.hour}?guild_id=${state.guildId}`, { method: "DELETE" });
      loadAnnouncements();
    };
    li.appendChild(span);
    li.appendChild(btn);
...
$("announcement-add-btn").onclick = async () => {
  const hour = parseInt($("announcement-hour").value);
  const url = $("announcement-url").value.trim();
  if (isNaN(hour) || hour < 0 || hour > 23 || !url) return;
  await api("/api/announcements", { method: "POST", body: JSON.stringify({ guild_id: state.guildId, hour, url }) });
  $("announcement-hour").value = "";
