...
class RadioBot(commands.Bot):
    ...
    async def _restore_radio(self):
        ...
        for cfg in guilds_cfg:
            ...
            try:
                ...
                player.default_eq_preset = cfg["eq_preset"]
    ...
    async def _check_hour_announcements(self):
        """Sprawdza, czy właśnie minęła pełna godzina i czy jest dla niej
        skonfigurowana zapowiedź ("Minęła właśnie ... w Bass FM"), i jeśli tak -
        przerywa aktualny utwór, żeby ją odtworzyć."""
        now = datetime.now(_TZ)
        key = (now.date(), now.hour)
        for player in self.players.all():
            if not player.radio_enabled or not player.connected:
                continue
            if player._playing_announcement:
                continue  # już gra zapowiedź
            if player._last_announced_key == key:
                continue  # ta godzina już zapowiedziana
            try:
                announcements = await self.db.get_hour_announcements(player.guild.id)
            except Exception:
                continue
            url = announcements.get(now.hour)
            if not url:
                continue
            player._last_announced_key = key
            try:
                await player.play_announcement(url)
                print(f"[radio] Zapowiedź godziny {now.hour} odtworzona na {player.guild.name}")
            except Exception as e:
                print(f"[radio] Błąd odtwarzania zapowiedzi godziny na {player.guild.name}: {e}")
    ...
    async def autoplay_watchdog(self):
        ...
        await self._check_hour_announcements()
